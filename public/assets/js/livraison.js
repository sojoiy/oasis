...
function ajouterLivreur(livraison, entite_ID)
{
...
function selectionnerLivreur(entite_ID, field, equipier_ID, type_ID)
{
	$("#id_intervenant").val(entite_ID);
	$("#id_equipier").val(equipier_ID);
    $("#typePiece").val(type_ID).change();
	
	$.ajax({
	        type:'POST',
	        url:'/intervenant/getinfo',
	        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			data: {
		        "id": entite_ID,
				"field": field
...
function rechercherPrestataire(keywords)
{
	if(keywords.length > 3)
	{
		...
		$.ajax({
		        type:'POST',
		        url:'/livraisons/rechercher-presta',
		        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				data: {
			        "keywords": keywords
			     },
...
function supprimerTransportPrincipal(livreur)
{
	$.ajax({
	        type:'POST',
	        url:'/livraisons/supprimertitulaireprincipal',
	        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			data: {
		        "id": livreur
		     },
...
function supprimerTransport(livreur)
{
	$.ajax({
	        type:'POST',
	        url:'/livraisons/supprimerpresta',
	        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			data: {
		        "id": livreur
		     },
...
function refresh_liste_titulaires(livraison)
{
	$.ajax({
		type: "POST",
		url: "/livraisons/liste_titulaires",
	    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
		data: {
	        "id": livraison
	     },
...
$(document).ready(function() {
	...
	$("#ajouterpresta").submit(function( event ) {
		...
		$.ajax({
			type: "POST",
			url: "/livraisons/addpresta",
			data: $("#ajouterpresta").serialize(),
				success: function(code){
					$('html,body').animate({scrollTop: 0}, 'slow');
					$('#zone_notify').html(code);
					refresh_liste_titulaires($('#livraisonID').val());
				},
