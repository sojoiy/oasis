			...
			<div class="card card-custom">
				...
				<form>
					...
					<table>
						<tr>
							<td>Intitulé</th>
							<td class="text-center"><input type="text" required class="form-control" onchange="saveTypeLivraison({{ $typeDossier->id }}, this.value, 'libelle');" size="16" name="" value="{{ $typeDossier->getField('libelle', $user->societeID) }}" /></td>
						</tr>
						<tr>
							<td>Mécanisme</td>
							<td class="text-center">
								<select name="mecanisme" onchange="saveTypeLivraison({{ $typeDossier->id }}, this.value, 'mecanisme_validation');refreshCas(this.value, {{ $typeDossier->id }})" class="form-control">
							...
							<td>Validation Niv 1</td>
								<td class="text-center" id="niv1_{{ $typeDossier->id }}">
									@if($typeDossier->niveau_validation >= 1 && $typeDossier->getField('mecanisme_validation', $user->societeID) > 2)
										<select required name="profil_1" onchange="saveTypeLivraison({{ $typeDossier->id }}, this.value, 'profil_1');"  class="form-control">
						...
						<tr>
							...
							<td class="text-center" id="niv2_{{ $typeDossier->id }}">
								@if($typeDossier->niveau_validation > 1 && $typeDossier->getField('mecanisme_validation', $user->societeID) > 2)
									<select required name="profil_2" onchange="saveTypeLivraison({{ $typeDossier->id }}, this.value, 'profil_2');"  class="form-control">
										<option value="">-- Sélectionnez un profil --</option>
										@foreach($profils as $profil)
						...
						<tr>
							...
							<td class="text-center" id="niv3_{{ $typeDossier->id }}">
								@if($typeDossier->niveau_validation > 2 && $typeDossier->getField('mecanisme_validation', $user->societeID) > 2)
									<select required name="profil_3" onchange="saveTypeLivraison({{ $typeDossier->id }}, this.value, 'profil_3');"  class="form-control">
										<option value="">-- Sélectionnez un profil --</option>
										@foreach($profils as $profil)
